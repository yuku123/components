package com.zifang.z.config.core.domain.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import org.springframework.stereotype.Service;
import com.zifang.z.config.core.domain.entity.ZUsers;
import com.zifang.z.config.core.domain.mapper.ZUsersMapper;
import com.zifang.z.config.core.domain.service.IZUsersService;

/**
 * 用户表 服务实现类
 * @author auto-generated
 */
@Service
public class ZUsersServiceImpl extends ServiceImpl<ZUsersMapper, ZUsers> implements IZUsersService {
}